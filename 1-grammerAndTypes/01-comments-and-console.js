/* 
COMMENTS

1- grammarAndTypes
 01-comments-and-console.js

*To Run code, type "ctrl+alt+n"*
 */

//value of a comment:
//communicates to anyone reading oure code

//types of comments:
//1) single-line comment

/*
  2) multiple-line comment
*/

//console.log:
//outputs some message to the console

console.log('this is a console.log message!');

console.log('you can seperate parts of a console message with commas,','just like this!');

console.log(13,15,17,18);

//console.log and debugging
//we can debug with console.log messages in 2 big ways;
//1)   we can check the states of some javascript 'thing'
//2)   we can cehck a process we have built with repeated console.log messages

//example 1:
var sampleName = 'Weston';
console.log(sampleName);

//example 2:
var sampleNum = 1;
console.log(sampleNum)
sampleNum++;
console.log(sampleNum)
sampleNum++;
console.log(sampleNum)